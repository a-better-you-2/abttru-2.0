    ...
    recipeDigestArray.forEach((recipeDigest, index) => {
      recipeDigest.forEach((nutrient, i) => {
        ...
        if (
          ...
          plotObjectsArray[index][0].data.values.push(
            nutrient.total / recipeYieldArray[index],
          );
          plotObjectsArray[index][0].data.labels.push(nutrient.label);

          if (nutrient.label === 'Fat') {
            nutrient.sub.forEach(fat => {
              plotObjectsArray[index][1].data.values.push(
                fat.total / recipeYieldArray[index],
              );
          ...
          plotObjectsArray[index][2].data.values.push(
            nutrient.total / recipeYieldArray[index],
          );
          ...
          // console.log(nutrient, i);
          // console.log(arrayDigest.slice(1, 5));
          plotObjectsArray[index][3].data.values.push(
            nutrient.total / recipeYieldArray[index],
          );
      ...
      return (
        <div>
          <div className="graphButtons">
            <Button id="pie" onClick={this.switchPlot} value={0}>
              Macros
            </Button>
            <Button id="pie" onClick={this.switchPlot} value={1}>
              Fats
            </Button>
            <Button id="pie" onClick={this.switchPlot} value={2}>
              Minerals
            </Button>
            <Button id="pie" onClick={this.switchPlot} value={3}>
              Vitamins
            </Button>
          </div>
          <Plot
