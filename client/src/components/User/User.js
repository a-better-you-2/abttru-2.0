...
class User extends React.Component {
  state = {
    data: [],
    user_id: "5aeb78c5e56e853d84a60546",
    recipe_id: "",
    name: "",
    password: "",
    risk_factor: "",
  ...
  getRecipes() {
    axios.get("https://api.edamam.com/search?q=beef&app_id=76461587&app_key=b829a690de0595f2fa5b7cb02db4cd99&from=0&to=5&calories=591-722&Diet=&Health=")
      .then(res => {
        console.log(res);
  ...
  render() {
    const recipeCard = this.state.data.map((data, index) =>
      <div className="panel panel-primary" key={data.recipe.uri}>
        ...
        <div className="panel-body">
          ...
          <p>{data.notes}</p>
          <Button className="save" id={data.recipe.uri} name={data.recipe.label} img={data.recipe.image} link={data.recipe.url} onClick={this.saveRecipe} data={this.state.data}>SAVE RECIPE</Button>
        </div>
      </div>
    )
